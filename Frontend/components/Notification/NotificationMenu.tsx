...
// core components
function notificationHandler(trigger: boolean) {
...
export default function NotificationMenu(props: any) {
  return (
    ...
    >
      <div
        className={"w-full h-full bg-gray-100 top-0 overflow-y-auto overflow-x-hidden fixed sticky-0"}
        id="chec-div"
        ref={node => {
          if (node && !props.notificationMenuOpen) {
            node.style.visibility = "visible";
          } else if (node && props.notificationMenuOpen) {
            node.style.visibility = "hidden";
          }
        }}
      >
        <div
          className={"w-full  absolute z-10 right-0 h-full overflow-x-hidden transform transition ease-in-out duration-700 translate-x-full "}
          id="notification"
          ref={node => {
            if (node && !props.notificationMenuOpen) {
              setTimeout(function () {
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                onClick={() => props.setNotificationMenuOpen(true)}
              >
                <svg
                  width="24"
                  height="24"
            ...
            <div className="w-full p-3 mt-4 bg-white rounded flex">
              ...
              <div className="pl-3">
                <p tabIndex={0} className="focus:outline-none text-sm leading-none">
                  Shipmet delayed for order<span className="text-indigo-700">
                    #25551</span
                  >
                </p>
                <p
